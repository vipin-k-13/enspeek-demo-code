...
export default function SingleSelectChart({ hasData = true, chartData, baseText, questionText, categories, totalRespondents, }: { questionId: string; hasData?: boolean; chartData: any[]; categories: string[]; baseText: string; questionText: string; totalRespondents: number }) {
  ...
  const options: Highcharts.Options = {
    ...
    xAxis: {
      ...
      labels: {
        autoRotation: [-0, -0],
        style: { fontSize: "10px", color: "var(--color-chart-axis)" },
      },
      lineColor: "var(--color-chart-line)",
      tickColor: "var(--color-chart-line)",
    },
    yAxis: {
      title: {
        text: "",
        style: { fontSize: "12px", color: "var(--color-chart-axis)" },
      },
      labels: { style: { fontSize: "12px", color: "var(--color-chart-axis)" } },
      gridLineColor: "var(--color-chart-grid)",
    },
    plotOptions: {
      column: {
        borderWidth: 0,
